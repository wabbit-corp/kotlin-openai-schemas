...
@Serializable @JvmInline value class ParagraphId(val value: Int)
...
sealed interface StoryElement {
    ...
    data class Paragraph(
        @SerialName("paragraphId") val id: ParagraphId,
        @Doc("The text of the paragraph") val text: String,
    ) : StoryElement

    @Serializable data class Image(val url: String) : StoryElement

    @Serializable data object Separator : StoryElement
}

@Serializable
data class Character(
    @Doc("The name of the character") val name: String,
    @Doc("The description of the character") val description: String,
    @Doc("The image of the character") val imageURL: String? = null,
)

@Serializable
data class Story(
    @Doc("The title of the story") val title: String,
    @Doc("The genre of the story") val genre: Genre,
    @Doc("The elements of the story: paragraphs and images") val elements: List<StoryElement>,
    @Doc("Characters in the story") val characters: Map<String, Character>,
)
...
sealed interface TSType {
    ...
    data class Object(val name: String, val fields: List<ObjectField>, val docString: String?) :
        TSType
        ...
        when (this) {
            ...
            is Object -> {
                ...
                val fields =
                    fields.joinToString("\n") {
                        val fieldDoc =
                            if (it.docString != null) "    // ${it.docString.trim()}\n" else ""
    ...
    //        is Array -> JsonObject(mapOf("type" to JsonPrimitive("array"), "items" to
    // elementType.toJsonSchema()))
    //        is Map -> JsonObject(mapOf("type" to JsonPrimitive("object"), "additionalProperties"
    // to valueType.toJsonSchema()))
    //        is Enum -> JsonObject(mapOf("type" to JsonPrimitive("string"), "enum" to JsonArray(
    //            values.map { JsonPrimitive(it.name) }
    //        )))
    //        is Object -> JsonObject(mapOf("type" to JsonPrimitive("object"), "properties" to
    // JsonObject(
...
private fun fromClassDescriptor(
    ...
    cache: MutableMap<Ref<SerialDescriptor>, TSType>,
                    ...
                    (0 until descriptor.elementsCount).map {
                        ...
                        val valueDocString =
                            descriptor
                                ...
                                .filterIsInstance<FunctionSchema.Doc>()
                                .firstOrNull()
                                ?.value
...
inline fun <reified T> def(
    cache: MutableMap<Ref<SerialDescriptor>, TSType> = mutableMapOf()
): TSType = def(serializer<T>().descriptor, cache)
